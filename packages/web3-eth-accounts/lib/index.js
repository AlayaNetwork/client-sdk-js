...
var Accounts = function Accounts(web3, hrp) {
    var _this = this;
    if (hrp === undefined || hrp === "" || hrp === null) {
        this.hrp = 'lat';
    }
    else {
        this.hrp = hrp;
...
Accounts.prototype._addAccountFunctions = function (account) {
    var _this = this;
    account.address = utils.toBech32Address(_this.hrp, account.address);
    // add sign functions
...
Accounts.prototype.signTransaction = function signTransaction(tx, privateKey, callback) {
    ...
    function signed(tx) {
        ...
        if ((tx.chain && !tx.hardfork) || (tx.hardfork && !tx.chain)) {
            error = new Error('When specifying chain and hardfork, both values must be defined. ' +
