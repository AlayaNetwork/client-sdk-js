...
function toVon(etherInput, unit) {
    ...
    if (ether === '.') {
        throw new Error(`[ethjs-unit] while converting number ${etherInput} to von, invalid value`);
    }
    // Split it into a whole and fractional part
    var comps = ether.split('.'); // eslint-disable-line
    if (comps.length > 2) {
        throw new Error(`[ethjs-unit] while converting number ${etherInput} to von,  too many decimal points`);
    }
    ...
    if (fraction.length > baseLength) {
        throw new Error(`[ethjs-unit] while converting number ${etherInput} to von, too many decimal places`);
    }
    while (fraction.length < baseLength) {
        fraction += '0';
    }
